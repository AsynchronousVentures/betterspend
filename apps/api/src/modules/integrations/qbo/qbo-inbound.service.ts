...
const CDC_ENTITY_TYPES = QBO_CATALOG_ENTITY_TYPES;

const QBO_WEBHOOK_ENTITY_TYPES = QBO_CATALOG_ENTITY_TYPES;

const QBO_WEBHOOK_OPERATIONS = ['create', 'update', 'delete', 'merge'] as const;
const qboOrganizationIdSchema = z.string().uuid();
const qboConnectionIdSchema = z.string().uuid();
...
const QBO_WEBHOOK_ENQUEUE_TIMEOUT_MS = 2_000;
...
export type QboMasterDataOperation =
  | {
      kind: 'snapshot';
      organizationId: string;
      entityTypes?: readonly QboSyncEntity[];
    }
  | { kind: 'cdc'; organizationId: string; lookbackDays?: number }
  | { kind: 'master-webhook'; event: QboWebhookEvent };

export interface QboMasterData {
  synchronize(operation: QboMasterDataOperation): Promise<QboSyncResult | void>;
  listMappings(organizationId: string, externalEntity?: string): Promise<unknown>;
  linkMapping(
    mappingId: string,
    organizationId: string,
  ...
  receiveWebhook(rawBody: Buffer, signature: string | undefined): Promise<unknown>;
  reconcileCatalogWebhook(
    organizationId: string,
    connectionId: string,
    realmId: string,
    entityName: QboSyncEntity,
  ): Promise<void>;
  processVendorMergeRecovery(
    input: Extract<QboCdcJobData, { kind: 'vendor-merge-recovery' }>,
  ): Promise<void>;
  ensureScheduledSync(organizationId: string): Promise<void>;
  enqueueInitialSync(
    organizationId: string,
    entityTypes?: readonly QboSyncEntity[],
  ): Promise<unknown>;
  enqueueCdcSweep(organizationId: string, lookbackDays?: number): Promise<unknown>;
}

export const QBO_MASTER_DATA = Symbol('QBO_MASTER_DATA');

type QboMasterDataPort = Pick<QboClientService, 'request'>;
 ...
 */
@Injectable()
export class QboInboundService implements OnModuleInit, OnModuleDestroy, QboMasterData {
  ...
  constructor(
    ...
    @Inject(QboClientService) private readonly qboClient: QboMasterDataPort,
  ...
  async synchronize(operation: QboMasterDataOperation): Promise<QboSyncResult | void> {
    switch (operation.kind) {
      case 'snapshot':
        return this.syncNow(operation.organizationId, operation.entityTypes);
      case 'cdc':
        return this.runCdcSweep(operation.organizationId, operation.lookbackDays);
      case 'master-webhook':
        return this.processWebhookEvent(operation.event);
      default:
  ...
  private async recordUnrecoverableVendorMerge(
    ...
    const { sourceId, targetId } = vendorMergeIds(event);
    const auditIdempotencyKey = qboRecoveryJobId('vendor-merge-audit', [
      connection.organizationId,
      connection.id,
      connection.realmId,
      event.entityName,
    ...
    try {
      ...
      await this.notifications.createIdempotent(
        `qbo-vendor-merge-recovery-failed:${auditIdempotencyKey}`,
        connection.organizationId,
        adminId,
  ...
  async listMappings(organizationId: string, externalEntity?: string) {
    if (externalEntity && !isSyncEntity(externalEntity)) {
      throw new BadRequestException(`Unsupported QBO master-data entity ${externalEntity}`);
    }
    const connection = await this.activeConnection(organizationId);
    if (!connection) return [];

    const mappings = await this.db.query.externalEntityMappings.findMany({
      where: (mapping, { and, eq }) =>
        and(
          ...
          inArray(mapping.externalEntity, QBO_SYNC_ENTITY_TYPES),
  ...
  async receiveWebhook(rawBody: Buffer, signature: string | undefined) {
    ...
    const events = parseQboWebhookEvents(payload).map((event) =>
      qboWebhookEventSchema.parse(event),
    );
    ...
    await withTimeout(
      enqueue,
      QBO_WEBHOOK_ENQUEUE_TIMEOUT_MS,
      'QBO webhook queue did not accept the event in time',
    );
  ...
  private async upsertMappingInTransaction(
    ...
    const values = {
      ...
      externalEntity: entityName,
      externalId,
      displayName: existingDelete ? existingDelete.displayName : definition.displayName(entity),
      syncToken: existingDelete ? existingDelete.syncToken : stringValue(entity.SyncToken),
      localEntity: definition.localEntity,
  ...
  private async handleVendorMerge(
    ...
    await this.db.transaction(async (transaction) => {
      ...
      if (source) {
        const sourceSyncedAt = latestQboTimestamp(source.syncedAt ?? undefined, providerUpdatedAt)!;
        await assertLock();
        const [updated] = await transaction
          .update(externalEntityMappings)
          .set({
      ...
      if (source?.localId) {
        if (target && !target.localId) {
          const targetSyncedAt = latestQboTimestamp(
            target.syncedAt ?? undefined,
            providerUpdatedAt,
          )!;
          await assertLock();
          const [updated] = await transaction
...
function sameQboTimestamp(left: Date | null | undefined, right: Date | null | undefined): boolean {
