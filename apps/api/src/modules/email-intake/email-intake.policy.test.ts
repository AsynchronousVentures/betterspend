...
describe('email intake policy', () => {
  ...
  it('rejects archives, encrypted PDFs, oversized files, and excess attachments', () => {
    ...
    const encryptedEscapedName = decideAttachment(
      {
        filename: 'locked-escaped.pdf',
        content: Buffer.from('%PDF-1.7\ntrailer\n<< /Size 3 /Encr#79pt 2 0 R >>\nstartxref\n0'),
      },
      0,
    );
    ...
    const oversizedTrailerDictionary = decideAttachment(
      {
        filename: 'oversized-trailer.pdf',
        content: Buffer.from(
          `%PDF-1.7\ntrailer\n<< /Padding (${'.'.repeat(65 * 1024)}) /Encrypt 2 0 R >>\nstartxref\n0`,
        ),
      },
      0,
    );
    ...
    const pdfRarPolyglot = decideAttachment(
      {
