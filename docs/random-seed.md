...
The normal command loads the small Acme Corp fixture used by demo mode. It is stable and upsert-only. The random command is opt-in, development-only, and inserts into that same fixed organization so demo-mode requests can see the generated data.
...
The graph is generated from deterministic IDs, business numbers, timestamps, values, and fake contact metadata. Webhook signing secrets are the deliberate exception, generated with cryptographic randomness at first persistence and never printed. The same seed and count can be rerun safely because conflict-safe inserts preserve an existing secret. A seed namespace may only be rerun with the count used by its first successful run. The command checks the generated requisition count for that seed in the Acme organization before writing. A count mismatch fails without changing the database. Choose a new seed when you need a different count.
