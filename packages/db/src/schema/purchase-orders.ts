...
export const purchaseOrders = pgTable(
  'purchase_orders',
  {
    ...
    blanketReleasedAmount: numeric('blanket_released_amount', { precision: 14, scale: 2 }).default(
      '0',
    ),
  ...
  (table) => ({
    idOrganization: uniqueIndex('purchase_orders_id_organization_id_unique').on(
      table.id,
...
export const poLines = pgTable('po_lines', {
  ...
  quantityReceived: numeric('quantity_received', { precision: 10, scale: 2 })
    .notNull()
    .default('0'),
  quantityInvoiced: numeric('quantity_invoiced', { precision: 10, scale: 2 })
    .notNull()
    .default('0'),
  ...
  contractComplianceDeltaPercent: numeric('contract_compliance_delta_percent', {
    precision: 8,
    scale: 4,
  }),
...
export const blanketReleases = pgTable('blanket_releases', {
  id: uuid('id').primaryKey().defaultRandom(),
  blanketPoId: uuid('blanket_po_id')
    .notNull()
    .references(() => purchaseOrders.id),
