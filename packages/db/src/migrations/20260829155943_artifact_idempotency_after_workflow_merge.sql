...
CREATE TABLE "artifact_operations" (
	...
	CONSTRAINT "artifact_operations_artifact_kind_check" CHECK ("artifact_operations"."artifact_kind" IS NULL OR "artifact_operations"."artifact_kind" IN ('requisition', 'rfq', 'message'))
);
--> statement-breakpoint
...
ALTER TABLE "artifact_notification_deliveries" ADD CONSTRAINT "artifact_notification_deliveries_operation_org_fk" FOREIGN KEY ("operation_id","organization_id") REFERENCES "public"."artifact_operations"("id","organization_id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
